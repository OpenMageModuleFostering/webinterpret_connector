...
class Webinterpret_Bridge2Cart_Helper
{
    ...
    public static function verifyRequest()
    {
        ...
        if (!defined('WI_VERIFY_SIGNATURE')) {
            if (self::loadBridgeConfig()) {
                if (defined('WI_VERIFY_SIGNATURE')) {
                    $validateSignature = (bool) WI_VERIFY_SIGNATURE;
                }
            }
        }
        ...
        // Get request id
        if (!isset($headers['Webinterpret-Request-Id'])) {
            throw new Exception('Request ID is required.');
        }
        if (isset($headers['Webinterpret-Request-Id'])) {
            $requestId = $headers['Webinterpret-Request-Id'];
        }

        // Get signature
        if (!isset($headers['Webinterpret-Signature'])) {
            throw new Exception('Signature is required.');
        }
        if (isset($headers['Webinterpret-Signature'])) {
            $signature = $headers['Webinterpret-Signature'];
        }
        $signature = base64_decode($signature);

        // Get signature version
        if (!isset($headers['Webinterpret-Signature-Version'])) {
            $version = 1;
        }
        if (isset($headers['Webinterpret-Signature-Version'])) {
            $version = $headers['Webinterpret-Signature-Version'];
        }
...
EOD;
    ...
    public static function sendResourceForbiddenResponse()
    {
        header('HTTP/1.0 403 Forbidden');
        header('Status: 403 Forbidden');
        echo "<!DOCTYPE HTML PUBLIC \"-//IETF//DTD HTML 2.0//EN\">" . PHP_EOL;
        echo "<html><head>" . PHP_EOL;
        echo "<title>403 Forbidden</title>" . PHP_EOL;
        echo "</head><body>" . PHP_EOL;
        echo "<h1>Forbidden</h1>" . PHP_EOL;
        echo "<p>You don't have permission to access this resource.</p>" . PHP_EOL;
    ...
    public static function handleRequest()
    {
        try {
            try {
                ...
                self::sendResourceForbiddenResponse();
                die();
            }
